...
**Returns:** `DocumentData` — The classification result. Inspect `.DocumentType.Id` for the resolved document-type identifier, `.DocumentType.Confidence` for the classifier's score, and `.SubDocuments` if document splitting produced multiple sections. `DocumentData` implements `IDocumentData`, so you can pass it — or any element of `.SubDocuments` — straight into `ExtractDocumentData(IDocumentData, …)` to chain into extraction without re-digitizing the file.
...
The `ExtractDocumentData` family has three overloads. The first two take a fresh document (path or `IResource`); the third takes an `IDocumentData` produced by a prior `ClassifyDocument` call (the whole result or one of its `SubDocuments`) and avoids re-digitizing the file.
...
### `IDocumentData<DictionaryData> ExtractDocumentData(IDocumentData classifiedDocument, string projectName, string projectVersionOrTag, string docType = null, int timeoutMs = 3_600_000)`

Extracts data from a previously classified document. Avoids re-digitizing the file because the document data already carries the digitized DOM.

**Parameters:**
- `classifiedDocument` (`IDocumentData`) — Output of a previous `ClassifyDocument` call, or one of its `SubDocuments` elements when document splitting produced multiple sections.
- `projectName` (`string`) — DU.Apps project name.
- `projectVersionOrTag` (`string`) — Version name or tag. Required.
...
Output of `ClassifyDocument`. Implements `IDocumentData` (which extends `IResource`) so it — like each of its `SubDocuments` — can be passed straight into `ExtractDocumentData(IDocumentData, …)` without re-loading the file.
...
- **`DocumentData` is itself an `IDocumentData` (and `IResource`).** Chain `ClassifyDocument` into `ExtractDocumentData(IDocumentData, …)` to avoid re-digitizing the file — pass the whole classification result, or iterate `SubDocuments` (typed `IDocumentData[]`) and pass each one when document splitting produced multiple sections. The classified document carries the digitized DOM into the extract call.
