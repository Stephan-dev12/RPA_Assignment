...
| `ExtractionModel` | IXP Model | InArgument | `string` | Conditional | | **IXP Solution Resource mode only** (`UseSolutionResource="True"`). The name of the IXP Model (IXP deployment / solution resource) to extract with, exactly as it appears in the designer's "IXP Model" dropdown / the Solution's resource catalog (e.g., `"IXP_StudioWebTests"`). Required in this mode — an empty value throws at the start of the folder-based extraction path: *"The IXP Model could not be resolved. Verify the IXP Model selection and folder path. (Parameter 'modelName')"*. (The **same** message with `(Parameter 'folderKey')` instead means the model is set but the folder was not resolved — see `FolderPath`.) Ignored when `UseSolutionResource="False"`. Set as a **literal attribute value**. |
| `FolderPath` | | InArgument | `string` | Conditional (required when authoring XAML directly / running headless) | | **IXP Solution Resource mode only.** Orchestrator folder where the IXP deployment is published. At runtime the model lookup is **folder-scoped**, so a non-null folder key is mandatory. The folder key comes from **either** this `FolderPath` **or** a Studio-written bindings override (`bindings_v2.json`), which Studio creates when you pick the model from the live "IXP Model" dropdown. **When authoring XAML directly (agents) or running headless (CLI/robot) there is no such binding, so `FolderPath` is required** — without it the runtime throws immediately: *"The IXP Model could not be resolved. Verify the IXP Model selection and folder path. (Parameter 'folderKey')"*. Must match the Orchestrator folder name **exactly** (a typo yields the same `folderKey` error). |
...
| `GenerateData` | Generate data type | `bool` | `false` | When `true`, generates a strongly-typed output with fields available under the `Data` property. When `false`, fields are retrieved via method calls by ID (using `DictionaryData`). **Agents should always set this to `False`** — `True` requires Studio's design-time JIT compilation which agents cannot trigger. Also, `GenerateData=True` is incompatible with `ModernDocumentTypeId="use_classification_result"`. Note: in IXP Solution Resource mode (`UseSolutionResource="True"`) this flag is ignored — extraction is late-bound and always emits `DictionaryData` output regardless of `GenerateData` or `T`. See Type Reference for details. |
| `UseSolutionResource` | Use Solution Resource | `bool` | `false` | When `True`, the activity extracts using an **IXP Model** picked from the Solution's resource catalog (`ExtractionModel`) instead of a Document Understanding project — the designer hides the project / version / document-type dropdowns and shows the "IXP Model" dropdown instead. In this mode `ProjectId`, `DocType`, `ModernDocumentTypeId`, `ExtractorName`, `ModernDocumentTypeName`, `ProjectName`, `ProjectTag`, `ProjectVersionNumber`, and `ProjectVersionName` are **not read** at runtime (set them to `{x:Null}` or omit them); the inputs that drive the run are `FileInput`, `ExtractionModel`, and a folder context (`FolderPath`, or a Studio-resolved binding — see `FolderPath`). This is a plain `bool` attribute (not an `InArgument`) — write `UseSolutionResource="True"`, not an expression. **Availability:** the toggle appears only inside a Solution project, in Studio Desktop `2026.0.197` STS and later (package `UiPath.DocumentUnderstanding.Activities` `3.4.0-preview` and later). |
...
### IXP Solution Resource Variant

Available from package `UiPath.DocumentUnderstanding.Activities` `3.4.0-preview` (Studio Desktop `2026.0.197` STS and later), **only inside a Solution project**. Instead of a Document Understanding project, extraction targets an **IXP Model** deployed to the Solution's resource catalog. Set `UseSolutionResource="True"`, put the IXP Model name in `ExtractionModel`, and set `FolderPath` to the Orchestrator folder where that model is published; leave every project / version / document-type property as `{x:Null}` (they are not read in this mode). `FileInput` + `ExtractionModel` + `FolderPath` drive the run.

> **`FolderPath` is required when authoring XAML directly.** In Studio, picking the model from the live "IXP Model" dropdown writes a bindings override that carries the folder, so a Studio-authored solution "just works" without a visible `FolderPath`. Agents writing raw XAML (and headless CLI/robot runs) have no such binding — omit `FolderPath` and the run fails at runtime with *"The IXP Model could not be resolved. Verify the IXP Model selection and folder path. (Parameter 'folderKey')"*.
...
<du:ExtractDocumentDataWithDocumentData
    ...
    DisplayName="Extract Document Data"
    ActivityMajorVersion="2"
    UseSolutionResource="True"
    ExtractionModel="{Your IXP Model Name}"
    FolderPath="{Orchestrator folder where the model is published}"
    ProjectId="{x:Null}"
    ProjectTag="{x:Null}"
    ProjectVersionNumber="{x:Null}"
    DocType="{x:Null}"
    ModernDocumentTypeId="{x:Null}"
    FileInput="[inputFile]"
    TimeoutInSeconds="[3600]"
    GenerateData="False"
    ExtractionResults="[extractionResults]">
...
Notes:
- `ExtractionModel` is the value the user gives you as their IXP solution / model name — set it verbatim (e.g., a user asking to *"process invoices using the IXP solution 'IXP_StudioWebTests'"* means `ExtractionModel="IXP_StudioWebTests"`). Ask the user for this name; there is no default and it cannot be inferred.
- **`FolderPath` must also be collected from the user** — the Orchestrator folder where the IXP model is published (e.g., `"first_folder"`). It has no default and cannot be inferred, and it must match the folder name in Orchestrator **exactly**. If the user doesn't provide it, ask. Omitting it (or a typo) throws the `folderKey` error above.
- Do **not** set `ProjectName`, `ClassifierId`, `DocType`, `ExtractorName`, `ModernDocumentTypeId`, or `ModernDocumentTypeName` in this mode. Studio's designer hides these fields when the toggle is on, and the runtime ignores them.
- Output is always `DictionaryData` (late-bound model, no design-time JIT), so `x:TypeArguments="dux:DictionaryData"` is the correct type and `GenerateData` has no effect. Read fields the usual way — `extractionResults.Data.GetFieldValue("total")`, `extractionResults.Data.Handler.GetTableValue("items")` — see [Reading Extraction Results](../overview.md#reading-extraction-results).
- **Troubleshooting** — *"The IXP Model could not be resolved. Verify the IXP Model selection and folder path. (Parameter 'folderKey')"* at runtime means `ExtractionModel` is set but `FolderPath` is empty or wrong. Set `FolderPath` to the deployment's Orchestrator folder (or, in Studio, re-select the model from the "IXP Model" dropdown so the binding is written). The same message with `(Parameter 'modelName')` means `ExtractionModel` itself is empty.
...
- **IXP Solution Resource mode** (`UseSolutionResource="True"`, package `3.4.0-preview`+, Studio Desktop `2026.0.197` STS+): extracts against an IXP Model from the Solution's resource catalog instead of a DU project. Set `ExtractionModel` to the IXP Model name **and `FolderPath` to the model's Orchestrator folder** (required for raw-XAML/headless authoring — without it the runtime throws the `folderKey` resolution error), and leave the project / version / document-type properties as `{x:Null}`. Only available inside a Solution project. Output is always `DictionaryData`. See the [IXP Solution Resource Variant](#ixp-solution-resource-variant) XAML example. This is distinct from a plain **IXP project** (previous bullet), which is still selected through the normal `ProjectName` + `"Default"` document-type flow.
